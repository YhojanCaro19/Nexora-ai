...
// documenta por qué se mantiene FIJO sin importar el auto-ajuste de FPS
// — si el count cambiara cada vez que QualityProvider sube/baja de nivel,
// el buffer de posiciones se regeneraría entero y el fondo "parpadearía"
// de densidad a mitad de la animación de entrada. Tampoco se espera que
// 23000 puntos aditivos (sin depthWrite, geometría trivial) sean el
// cuello de botella real en mobile: el Canvas ya renderiza a dpr:1 desde
// el arranque en este tier (QualityProvider → INITIAL_LEVEL_BY_TIER.mobile
// = 'Low', el piso de la escala), que es la única palanca real que el
// auto-ajuste de FPS tiene disponible — y ya está al mínimo.
const MOBILE_STAR_COUNT = Math.round(DEFAULT_STAR_COUNT * 1.15);
...
export function MobileWordmarkScene() {
  ...
  useFrame((frameState, delta) => {
    ...
    if (phase === 'appear') {
      ...
      // pantalla, la posición no se mueve durante esta fase.
  ...
  return (
    <>
      {/* Mismo fondo que desktop (Environment.tsx) — mismos radios 30–70,
          solo +15% de densidad (ver MOBILE_STAR_COUNT arriba). */}
      ...
      >
        <WordmarkModel sparksEnabled={sparksEnabled} />
      </group>
    </>
  );
}
