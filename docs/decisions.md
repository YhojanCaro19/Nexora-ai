...
La pantalla de Perfil muestra el plan del negocio con lo que incluye cada mes (conversaciones del agente, campañas, imágenes con IA, créditos de respaldo, topes de negocios/colaboradores) leído tal cual de la tabla `plans` que administra el superadmin — más el consumo del agente del propio negocio (`getAgentUsageForBusiness`, `agentUsageService.ts`): conversaciones (hilos de `conversations`), mensajes respondidos (`replyCount` = filas de `agent_usage_log` = turnos del agente, NO hilos), tokens usados y última actividad. **Sin precios en USD ni costo estimado** — eso es solo para el panel de superadmin (`/superadmin/consumo`). Al dueño se le muestra volumen (tokens), no plata. Un colaborador no ve el bloque de consumo (RLS de `agent_usage_log` = `is_business_admin`, y la página de colaborador pasa `agentUsage={null}`).
