...
## Una cuenta = un negocio
No hay "vincular varios negocios a una cuenta" ni selector de negocio. `processApprovedPayment` trata un segundo pago del mismo correo como **renovación**, nunca crea un segundo negocio; `createManualPendingRegistration` lo rechaza explícito. Se quitó la columna informativa `plans.max_businesses` (`docs/sql/remove-max-businesses.sql`) y toda su copy — nunca se hacía cumplir y solo confundía. Si algún día se quiere multi-negocio real (agencias), es una decisión y un diseño nuevos, no algo que ya esté "medio hecho".
...
## Perfil → "Plan y facturación": el dueño ve cupos + tokens, nunca dinero
La pantalla de Perfil muestra el plan del negocio con lo que incluye cada mes (conversaciones del agente, campañas, imágenes con IA, créditos de respaldo, tope de colaboradores) leído tal cual de la tabla `plans` que administra el superadmin — más el consumo del agente del propio negocio (`getAgentUsageForBusiness`, `agentUsageService.ts`): conversaciones (hilos de `conversations`), mensajes respondidos (`replyCount` = filas de `agent_usage_log` = turnos del agente, NO hilos), tokens usados y última actividad. **Sin precios en USD ni costo estimado** — eso es solo para el panel de superadmin (`/superadmin/consumo`). Al dueño se le muestra volumen (tokens), no plata. Un colaborador no ve el bloque de consumo (RLS de `agent_usage_log` = `is_business_admin`, y la página de colaborador pasa `agentUsage={null}`).

## Historial de seguridad = línea de tiempo personal unificada
Perfil → "Historial de seguridad" fusiona inicios de sesión (`user_login_events`) + eventos propios (`profile_security_events`), ordenados por fecha. Los `event_type` incluyen accesos sensibles a datos del negocio: agregar/editar/desactivar/reactivar/eliminar colaboradores, descargar un reporte, pedir cambio de cuenta de acceso — cada uno queda en el historial de QUIEN lo hizo (`auth.uid() = user_id`), no en un panel de auditoría del negocio. Se escriben con service role desde los server actions correspondientes vía `logProfileSecurityEvent`. Migración: `docs/sql/profile-security-events.sql`.

## Multi-idioma es bloqueador de despliegue (2026-09-01)
El i18n (next-intl, cookie `LOCALE`) hoy solo cubre landing + login. El dashboard entero sigue en español fijo. AVENTHRA no sale a producción hasta que el panel esté traducido de verdad (ES + EN con paridad de claves y el toggle funcionando de punta a punta) y el andamiaje listo para más idiomas. En Perfil la sección "Preferencias" se renombró "Idioma"; el toggle ES/EN se queda con una nota honesta de que la traducción completa llega al final. Ver `CLAUDE.md` (bloqueadores duros) y `docs/sql`/memoria del checklist de pre-deploy.

## Turno/cita completado → pedido finalizado (las mesas no)
Un turno o cita con precio, al marcarse **completado** en el módulo de Reservas (desde la agenda o por el cron de auto-completado), genera un pedido en `orders` con estado `picked_up` — aparece en Pedidos → finalizados, cuenta en "ventas de hoy" y en los reportes. `orders.created_at` = el momento en que se marcó completado (decisión del usuario: la venta cae el día en que el negocio la cerró, no la hora de fin del turno). Idempotente vía `reservations.order_id`. `recordCompletedAppointmentAsOrder` en `reservationService.ts`; `getOrders` embebe la reserva de origen (`reservations(id, starts_at, kind)`) para marcar el pedido con un ícono de reloj y una fila "Origen: Turno de Reservas".

**Las reservas de MESA no crean pedido** — no generan ingreso; lo que el cliente consume en la mesa es un pedido normal aparte (`tomar_pedido`). Solo los turnos/citas con un servicio con precio > 0 se registran. Un turno sin precio tampoco genera pedido. El pedido espejo nace ya finalizado y no se gestiona desde Pedidos (se gestiona desde Reservas): estados `no_show`/`cancelled` no crean nada porque el pedido solo se materializa al completar.

## "Comparativa por período" es un módulo propio, no una vista de Reportes
Estaba como una opción dentro del chooser de Reportes; el usuario pidió sacarla a su propio módulo en el nav (`ADMIN_NAV` → grupo "Negocio", `/admin/comparativa`, icono `TrendingUp`). Reportes queda solo con Personalizar PDF · Descargar reporte de hoy · Historial de reportes. El componente `sales-comparison.tsx` y su server action `getSalesRangeSummaryAction` (wrapper de `getSalesSummaryForRange`) se movieron a `app/(dashboard)/admin/comparativa/`. No es asignable a colaboradores (mismo criterio que Reportes: expone ventas/ingresos agregados).

## Sesión atada al dispositivo (defensa contra sesión portada)
La sesión de Supabase vive en cookies httpOnly y `proxy.ts` ya bloquea cualquier ruta del panel sin sesión válida — copiar una URL nunca da acceso. Capa extra (pedida por el usuario): al iniciar sesión se guarda una cookie httpOnly `av_dev` con el SHA-256 del User-Agent (`lib/auth/session-guard.ts`, Web Crypto para correr en `proxy.ts`). En cada request al panel, `proxy.ts` compara esa huella con la del navegador actual; si no coincide (cookies copiadas a otro navegador/equipo) → `signOut({scope:'global'})` real + evento `session_device_mismatch` en `profile_security_events` (se ve en rojo en Perfil → Historial de seguridad) + redirect a `/login`. Si la cookie falta (sesión previa a la función) se ata al dispositivo actual sin cerrar nada. Un login explícito siempre re-ata, así que cambiar de navegador legítimamente solo cuesta volver a entrar. Límite conocido: si el atacante además falsea su User-Agent idéntico al de la víctima, la huella coincide — es una barrera proporcional (pyme; Google cubre la cuenta), no anti-robo perfecto.

## Login por pestaña + "la URL no loguea"
Decisión del usuario (super-regla): copiar una URL del panel y abrirla en otra pestaña —aunque sea el MISMO navegador— debe mandar a /login. No hay primitiva HTTP per-tab (las cookies se comparten), así que se hace con `sessionStorage` (por-pestaña): el callback de Google emite `av_tab_grant` (httpOnly, 1 uso, 2 min), `TabSessionGuard` en `DashboardShell` lo canjea en `/api/auth/claim-tab` y marca la pestaña; refresh la mantiene, cualquier otra pestaña va a /login (y puede re-loguearse con 1 clic de Google). El guard no pinta el panel hasta canjear. Es enforcement client-side (server no distingue pestañas); la garantía dura la dan cookies httpOnly + proxy.ts + device binding. Ver `docs/security-hardening.md` §1.

## Consumo del agente en Perfil: ventana de 30 días, no histórico
El "Tokens usados" de Perfil → Plan mostraba el acumulado histórico y por eso nunca cuadraba con el panel de Anthropic (que muestra ventanas móviles / reset mensual). Se acotó `getAgentUsageForBusiness` a los últimos 30 días (`OWN_AGENT_USAGE_WINDOW_DAYS`) y se rotula "· últimos 30 días". El total de tokens incluye input + output + caché leído + caché escrito (el volumen real que Anthropic factura por llamada, sumado por ronda de tool en `runAgentTurn`) — de ahí una nota en la UI explicando que es más alto que "el mensaje final".
