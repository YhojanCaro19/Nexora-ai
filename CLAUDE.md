...
- **Dominio propio verificado:** para Resend (OTP + reportes a destinatarios reales) y para las URLs de producción. Hoy Resend está en sandbox — ver `docs/` y la memoria del bloqueador de Resend.
  - ⚠️ **Si el dominio cambia** (el usuario cree que lo hará): rehacer TODO lo que dependa de él antes de desplegar — `NEXT_PUBLIC_APP_URL` (env local + Vercel); verificación del dominio en Resend + `RESEND_FROM_EMAIL` + DNS (SPF/DKIM/DMARC); **app de Meta**: App domains, URLs legales (`/privacidad` `/terminos` `/borrado-de-datos`), **URI de redireccionamiento de OAuth válidos** (`/api/auth/meta/callback`), URL del webhook (`/api/webhooks/meta`); Google/TikTok OAuth redirect URIs; Wompi (redirect + webhook URL); cualquier `redirect_uri`/callback registrado en un tercero. Checklist detallado en `docs/channels-setup-checklist.md`.
...
- **TikTok Ads: POSPUESTO al post-lanzamiento** (decidido 2026-09-07 — el usuario
  todavía no cumple varios de los requisitos del trámite: documentos legales de la
  empresa para la verificación del Business Center, entre otros). No bloquea el
  despliegue. La landing hoy no menciona TikTok en ningún lado, así que no hace falta
  marcarlo como "en camino" — si en algún momento se agrega, debe quedar así hasta que
  esto se retome. Checklist y trámites en `docs/tiktok-ads-setup-checklist.md`.
...
- **Crons de Vercel:** hay 6 (`vercel.json`, todos `0 * * * *` — cada uno decide internamente si hoy le toca hacer algo): `/api/cron/daily-reports` (reporte diario a medianoche local de cada país), `/api/cron/reservation-reminders` (recordatorio de confirmación 1 día antes de cada reserva/turno), `/api/cron/channel-token-health` (detecta tokens de canal muertos), `/api/cron/marketing-metrics` (sync diario de métricas de Meta Ads), `/api/cron/monthly-stats-snapshot` (día 1 del mes, guarda el snapshot de `platform_monthly_stats` — Superadmin → Estadísticas), `/api/cron/plan-renewal-reminders` (avisa por correo cuando a un negocio le queda ≤5 días para el vencimiento de su mensualidad, o ya venció — solo informativo, no corta el acceso). Solo corren en un deployment de producción en Vercel, nunca en local. Antes de dar por buenas estas features: (1) `CRON_SECRET` en las env vars del proyecto en Vercel, (2) plan **Vercel Pro** (Hobby: máx. 2 crons y corren máx. 1 vez/día, no puntual — con 6 crons definidos ya no alcanza), (3) probado end-to-end cada uno.
